...
def process(file_path):
    ...
    if args.skip != None:
        ...
        ID = re.compile('\*\*\*')
        logging.debug('ID preamble-end: %s', \
                      f[ID.search(f).start():ID.search(f, ID.search(f).end()).end()])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-H', '--histogram', \
                        help='Optional histogram of the frequencies: digit yes   if you want')
    parser.add_argument('-s', '--skip', \
                        help='Digit skip if you want to skip preamble and licence')
    parser.add_argument('-S', '--statistic', \
                        help='Digit yes if you want basic statistic')
